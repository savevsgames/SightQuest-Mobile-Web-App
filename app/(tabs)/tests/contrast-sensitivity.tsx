...
const TEST_DURATION = 5000; // 5 seconds per letter
...
const getLetterColor = (contrastLevel: number, isDark: boolean): string => {
  const bg = isDark ? 20 : 240; // background luminance estimate (0–255)
  const contrast = contrastLevel / 100; // convert to 0–1
  const fgLuminance = isDark
...
export default function ContrastSensitivityTest() {
  ...
  const letterStyle = useAnimatedStyle(() => {
    const contrast = CONTRAST_LEVELS[currentContrastIndex];
    const color = getLetterColor(contrast, isDark);
  ...
  const handleInput = async (text: string) => {
    ...
    // End test after enough reversals
      ...
      <View style={styles.content}>
        {testStatus === 'ready' && (
          <View style={styles.instructionsContainer}>
            ...
            >
              Letters will appear with decreasing contrast. Type each letter as
              soon as you can read it. The test measures your ability to detect
        ...
        {testStatus === 'running' && (
          <View style={styles.testContainer}>
            ...
            <Text style={[styles.progress, { color: theme.textSecondary }]}>
              Letter {currentLetterIndex + 1} of {NUM_TEST_LETTERS}
            </Text>
            <Text
              style={[styles.contrastLevel, { color: theme.textSecondary }]}
            >
              Contrast Level: {CONTRAST_LEVELS[currentContrastIndex]}%
            </Text>
          </View>
        )}
